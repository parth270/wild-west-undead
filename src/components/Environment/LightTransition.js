...
function LightTransition({ darkMode }) {
  ...
  const animateTransition = (elapsedTime) => {
    ...
    if (ambientLightRef.current && pointLightRef.current) {
      ambientLightRef.current.intensity += speed * (targetAmbientIntensity - ambientLightRef.current.intensity);
      pointLightRef.current.intensity += speed * (targetPointIntensity - pointLightRef.current.intensity);
  ...
  return (
    <>
      ...
      <Canvas style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', visibility: isLoading ? 'hidden' : 'visible' }}>
        <Sky sunPosition={pointLightRef.current ? pointLightRef.current.position.toArray() : [0, 5, 10]} />
        <Stars />
